...
Because multiple versions of lit-html might be loaded on the page, the icon can also be a function
which receives the html tag:
...
On IE11 and some versions of Edge, SVG elements are focusable by default. Setting `focusable="false"` on the SVG prevents this.
